...
public class Programmer3 extends Employee3 {
	...
	public boolean matches(String firstName, String lastName, Date birthFrom, Date birthTo, Date employmentFrom,
		...
		boolean match = super.matches(firstName, lastName, birthFrom, birthTo, employmentFrom, employmentTo,
				sallaryFrom, sallaryTo);
		if (mapProgrammingLanguages != null && !mapProgrammingLanguages.isEmpty()) {
			for (String key : mapProgrammingLanguages.keySet()) {
				if (this.mapProgrammingLanguages.containsKey(key)
						&& this.mapProgrammingLanguages.get(key) == mapProgrammingLanguages.get(key)) {
					match = true;
				} else {
					return false;
				}
